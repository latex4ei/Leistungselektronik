...
\sectionbox{
	\subsection{Snubber-Schaltung}

	\includegraphics[width=5cm]{img/04-Snubber.pdf}
...
\section{Verlustleistung und Kühlung}
\sectionbox{
	...
	$p_D = u i = U_S i + r_D i^2$

	$P_D = \frac{1}{D} \int_0^T p_D \diff t = U_S \frac{1}{T} \int_0^T i \diff t + r_D \frac{1}{T} \int_0^T i^2 \diff t $
...
\sectionbox{
	\subsection{Transiente Wärmewiderstände}
		$Z_{\ir thi} (t) = R_{\ir thi} \left( 1 - e^{\frac{-t}{\tau_{\ir thi}}} \right)$
}
% Ende der Spalten
\end{multicols*}
